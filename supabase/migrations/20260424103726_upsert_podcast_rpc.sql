-- Atomic OR-match + upsert for the podcast table in ONE roundtrip.
-- Matches by any of: name, itunes_id, spotify_id, castro_id (first-wins by id).
-- Fills nulls only on update; preserves keeper's existing non-null metadata.
...
GRANT EXECUTE ON FUNCTION public.upsert_podcast(jsonb) TO service_role;
